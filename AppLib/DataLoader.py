from AppLib import ConfigLoader
...
def read_accounts(spark, env, enableHive, HiveDB):
    run_filter = ConfigLoader.get_data_filter(env, "account.filter")

    if enableHive:
        return spark.sql("select * from " + HiveDB + ".accounts ").where(run_filter)
    else:
        return spark.read \
            .format("csv") \
            .option("header", "true") \
...
def read_parties(spark, env, enableHive, HiveDB):
    run_filter = ConfigLoader.get_data_filter(env, "parties.filter")

    if enableHive:
        return spark.sql("select * from " + HiveDB + ".parties ").where(run_filter)
    else:
        return spark.read \
            .format("csv") \
            .option("header", "true") \
...
def read_addresses(spark, env, enableHive, HiveDB):
    run_filter = ConfigLoader.get_data_filter(env, "address.filter")

    if enableHive:
        return spark.sql("select * from " + HiveDB + ".address ").where(run_filter)
    else:
        return spark.read \
            .format("csv") \
            .option("header", "true") \
